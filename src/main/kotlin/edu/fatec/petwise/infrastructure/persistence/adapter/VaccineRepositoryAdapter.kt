...
class VaccineRepositoryAdapter(
    ...
    override fun findByOwnerId(ownerId: UUID): List<Vaccine> {
        return jpaVaccineRepository.findByOwnerId(ownerId).map { it.toDomain() }
    }

    override fun countByOwnerId(ownerId: UUID): Int {
        return jpaVaccineRepository.countByOwnerId(ownerId)
    }
