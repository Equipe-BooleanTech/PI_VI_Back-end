...
@Service
class GetAllVaccinesByOwnerUseCase(
    private val vaccineRepository: VaccineRepository,
    private val petRepository: PetRepository,
    private val userRepository: UserRepository
) {
    ...
    fun execute(ownerId: UUID): List<VaccineResponse> {
        logger.info("Buscando todas as vacinas do tutor: $ownerId")

        return vaccineRepository.findByOwnerId(ownerId).map { vaccine ->
    ...
    fun count(ownerId: UUID): Int {
        return vaccineRepository.countByOwnerId(ownerId)
    }
}
