...
class CreateAppointmentUseCase(
    ...
    fun execute(request: CreateAppointmentRequest, ownerId: UUID): AppointmentResponse {
        ...
        val appointment = Appointment(
            ...
            consultaDate = request.consultaDate.toString(),
            consultaTime = request.consultaTime.toString(),
...
class GetAppointmentsByOwnerUseCase(
    ...
    fun count(ownerId: UUID): Int {
        return appointmentRepository.countByOwnerId(ownerId)
    }
}

@Service
class GetUpcomingAppointmentsByOwnerUseCase(
    private val appointmentRepository: AppointmentRepository,
    private val petRepository: PetRepository,
    private val userRepository: UserRepository
) {
    ...
    fun execute(ownerId: UUID): List<AppointmentResponse> {
        logger.info("Buscando consultas futuras do owner: $ownerId")
        val owner = userRepository.findById(ownerId)!!
        
        return appointmentRepository.findByOwnerId(ownerId)
            .filter { it.status == edu.fatec.petwise.domain.enums.ConsultaStatus.SCHEDULED }
            .map { appointment ->
                val pet = petRepository.findById(appointment.petId)!!
    ...
    fun count(ownerId: UUID): Int {
        return appointmentRepository.findByOwnerId(ownerId)
            .count { it.status == edu.fatec.petwise.domain.enums.ConsultaStatus.SCHEDULED }
    }
}
...
class GetAppointmentsByPetUseCase(
    ...
    fun count(petId: UUID, ownerId: UUID): Int {
        val pet = petRepository.findById(petId)
            ?: throw EntityNotFoundException("Pet", petId)
        ...
        return appointmentRepository.countByPetId(petId)
    }
}
...
class UpdateAppointmentStatusUseCase(
    ...
    fun execute(id: UUID, statusName: String, ownerId: UUID, notes: String? = null): AppointmentResponse {
        logger.info("Atualizando status da consulta ID: $id para $statusName")
        ...
        val newStatus = try {
            edu.fatec.petwise.domain.enums.ConsultaStatus.valueOf(statusName.uppercase())
        } catch (e: IllegalArgumentException) {
            throw BusinessRuleException("Status inválido: $statusName")
        }

        val updated = when (newStatus) {
            edu.fatec.petwise.domain.enums.ConsultaStatus.IN_PROGRESS -> appointment.start()
            edu.fatec.petwise.domain.enums.ConsultaStatus.COMPLETED -> appointment.copy(
                status = newStatus,
                updatedAt = java.time.LocalDateTime.now()
            )
            edu.fatec.petwise.domain.enums.ConsultaStatus.CANCELLED -> appointment.cancel()
            edu.fatec.petwise.domain.enums.ConsultaStatus.RESCHEDULED -> appointment.copy(
                status = newStatus,
                updatedAt = java.time.LocalDateTime.now()
            )
        ...
        val saved = appointmentRepository.save(updated)
        logger.info("Status atualizado com sucesso. ID: $id -> ${saved.status}")
...
class GetUpcomingAppointmentsUseCase(
    ...
    fun execute(ownerId: UUID): List<AppointmentResponse> {
        ...
        val statuses = listOf(
            edu.fatec.petwise.domain.enums.ConsultaStatus.SCHEDULED,
            edu.fatec.petwise.domain.enums.ConsultaStatus.RESCHEDULED
        )
        ...
        return appointmentRepository.findByOwnerId(ownerId)
            ...
            .map { appointment ->
                val pet = petRepository.findById(appointment.petId)!!
                appointment.toResponse(petName = pet.name, veterinarianName = "")
            }
    }
}
