...
import edu.fatec.petwise.application.dto.VaccineListResponse
...
class VaccineController(
    ...
    private val getAllVaccinesByOwnerUseCase: GetAllVaccinesByOwnerUseCase,
    ...
    ): ResponseEntity<VaccineListResponse> {
        val ownerId = UUID.fromString(authentication.name)
        logger.info("Requisição para buscar todas as vacinas do tutor: $ownerId")
        val vaccines = getAllVaccinesByOwnerUseCase.execute(ownerId)
        val total = getAllVaccinesByOwnerUseCase.count(ownerId)
        val response = VaccineListResponse(
